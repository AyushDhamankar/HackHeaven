...
function UserFeedback({ state }) {
  ...
  const getData = async () => {
    try {
      ...
      console.log(email);
      const filteredPosts = data.postList.filter(post => post.email === email && post.solutionposts.length == 1);
        ...
        <div class="container px-5 py-24 mx-auto" style={{ minHeight: "90vh" }}>
          ...
          >
            {data !== "" &&
              ...
              <Link to={`/feedbackpost/${data.id}`} class="text-indigo-400 inline-flex items-center md:mb-2 lg:mb-0">Rate Now
